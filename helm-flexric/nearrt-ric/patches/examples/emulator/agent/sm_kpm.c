...
/* New KPM metrics — TB layer (3GPP TS 28.552 section 5.1.1.6) */

static
meas_record_lst_t fill_TB_TotNbrDlInitial(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_IntialErrNbrDl(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_TotNbrDl_sum(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_ResidualErrNbrDl(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_TotNbrUlInit(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_ErrNbrUlInitial(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_TotNbrUl_sum(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_TB_ResidualErrNbrUl(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}

/* New KPM metrics — RLC layer (3GPP TS 28.552 section 5.1.3) */
...
static
meas_record_lst_t fill_DRB_PacketLossRateUl(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_DRB_RlcSduLatencyDl(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}

/* New KPM metrics — RRC layer (3GPP TS 28.552 section 5.1.1.8) */

static
meas_record_lst_t fill_RRC_ConnMean(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
static
meas_record_lst_t fill_RRC_ConnMax(ue_id_e2sm_t const* ue)
{
  assert(ue != NULL);
  return fill_rnd_int_data();
}
...
const kv_measure_t lst_measure[] = {
  ...
  (kv_measure_t){.key = "TB.TotNbrDlInitial", .value = fill_TB_TotNbrDlInitial },
  (kv_measure_t){.key = "TB.IntialErrNbrDl", .value = fill_TB_IntialErrNbrDl },
  (kv_measure_t){.key = "TB.TotNbrDl.sum", .value = fill_TB_TotNbrDl_sum },
  (kv_measure_t){.key = "TB.ResidualErrNbrDl", .value = fill_TB_ResidualErrNbrDl },
  (kv_measure_t){.key = "TB.TotNbrUlInit", .value = fill_TB_TotNbrUlInit },
  (kv_measure_t){.key = "TB.ErrNbrUlInitial", .value = fill_TB_ErrNbrUlInitial },
  (kv_measure_t){.key = "TB.TotNbrUl.sum", .value = fill_TB_TotNbrUl_sum },
  (kv_measure_t){.key = "TB.ResidualErrNbrUl", .value = fill_TB_ResidualErrNbrUl },
  (kv_measure_t){.key = "DRB.RlcPacketDropRateDl", .value = fill_DRB_RlcPacketDropRateDl },
  (kv_measure_t){.key = "DRB.PacketLossRateUl", .value = fill_DRB_PacketLossRateUl },
  (kv_measure_t){.key = "DRB.RlcSduLatencyDl", .value = fill_DRB_RlcSduLatencyDl },
  (kv_measure_t){.key = "RRC.ConnMean", .value = fill_RRC_ConnMean },
  (kv_measure_t){.key = "RRC.ConnMax", .value = fill_RRC_ConnMax },
  };
  // 3GPP TS 28.552
