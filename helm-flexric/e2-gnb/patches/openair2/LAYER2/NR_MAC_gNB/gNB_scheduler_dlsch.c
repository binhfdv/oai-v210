    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    /* Per-slice PRB enforcement */
    {
      ...
      for (int lci = 0; lci < sched_ctrl_ue->dl_lc_num; lci++) {
        ...
        if (lc >= 4) {
          slice_found = nr_slice_table_lookup(sched_ctrl_ue->dl_lc_nssai[lc].sst,
    ...
    /* Clamp min_rbSize to max_rbSize: when a slice restricts max_rbSize below
     * the global minimum, nr_find_nb_rb would otherwise fall back to
     * nb_rb_min RBs, silently bypassing the slice PRB limit. */
    const uint16_t eff_min_rbSize = max_rbSize < min_rbSize ? max_rbSize : min_rbSize;
