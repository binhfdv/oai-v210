...
static bool nssai_matches(nssai_t a_nssai, uint8_t b_sst, const uint32_t *b_sd)
{
  ...
  if (b_sd == NULL) {
    /* No SD specified in the filter: match on SST only (wildcard SD per 3GPP TS 23.003). */
...
/* Metrics available at gNB-DU or mono-gNB: RLC, MAC/TB, and RRC connection count.
   3GPP TS 28.552 sections 5.1.1.6 (TB), 5.1.3 (RLC), 5.1.1.8 (RRC). */
...
/* Metrics available at mono-gNB: PDCP (CU-UP side) + all DU-side metrics. */
