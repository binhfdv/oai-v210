...
static void fill_rc_control(ran_func_def_ctrl_t* ctrl)
{
...
#ifdef NGRAN_GNB_DU
  /* ---- Style 2: Radio Resource Allocation Control (Action 6: Slice-level PRB quota) ---- */
  seq_ctrl_style_t* ctrl_style2 = &ctrl->seq_ctrl_style[1];

  ctrl_style2->style_type = 2;
  const char control_name2[] = "Radio Resource Allocation Control";
  ctrl_style2->name = cp_str_to_ba(control_name2);
  ...
  ctrl_act2->id = 6;
  const char control_act_name2[] = "Slice-level PRB quota";
  ctrl_act2->name = cp_str_to_ba(control_act_name2);
  ctrl_act2->sz_seq_assoc_ran_param = 0;
  ctrl_act2->assoc_ran_param = NULL;
...
#ifdef NGRAN_GNB_DU
/* DU only advertises Style 2 / Action 6: Slice-level PRB quota */
static e2sm_rc_func_def_t fill_rc_ran_def_du(void)
{
  ...
  s2->style_type = 2;
  const char ctrl_name[] = "Radio Resource Allocation Control";
  s2->name = cp_str_to_ba(ctrl_name);
  s2->hdr  = FORMAT_1_E2SM_RC_CTRL_HDR;
  s2->msg  = FORMAT_1_E2SM_RC_CTRL_MSG;
  s2->call_proc_id_type = NULL;
  s2->out_frmt = FORMAT_1_E2SM_RC_CTRL_OUT;
  s2->sz_ran_param_ctrl_out = 0;
  s2->ran_param_ctrl_out = NULL;

  s2->sz_seq_ctrl_act = 1;
  s2->seq_ctrl_act = calloc(1, sizeof(seq_ctrl_act_2_t));
  assert(s2->seq_ctrl_act != NULL && "Memory exhausted");
  s2->seq_ctrl_act[0].id = 6;
  const char act_name[] = "Slice-level PRB quota";
  s2->seq_ctrl_act[0].name = cp_str_to_ba(act_name);
  s2->seq_ctrl_act[0].sz_seq_assoc_ran_param = 0;
  s2->seq_ctrl_act[0].assoc_ran_param = NULL;
...
static const fp_rc_func_def ran_def_rc[END_NGRAN_NODE_TYPE] =
{
  ...
  NULL,                    // ngran_eNB_DU
#ifdef NGRAN_GNB_DU
  fill_rc_ran_def_du,      // ngran_gNB_DU - Style 2/Action 6 slice PRB quota
#else
  fill_rc_ran_def_null,    // ngran_gNB_DU - fallback
#endif
  NULL,                    // ngran_eNB_MBMS_STA
  fill_rc_ran_def_cucp,    // ngran_gNB_CUCP
  fill_rc_ran_def_null,    // ngran_gNB_CUUP - at the moment, no Service is supported
};
...
//  assert(data->type == RAN_CONTROL_CTRL_V1_03 );
  ...
  if (ctrl->hdr.frmt_1.ctrl_act_id == 6) {
    ...
    fflush(stdout);
    return add_mod_rc_slice(ctrl);
  }
#endif

  assert(ctrl->hdr.frmt_1.ctrl_act_id == 2 && "Currently only QoS flow mapping (2) and slice PRB quota (6) supported");
